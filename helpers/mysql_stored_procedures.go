...
BEGIN
    ...
    DECLARE spaces_cursor CURSOR FOR SELECT id FROM spaces WHERE name LIKE '{{.Prefix}}-space-%' ORDER BY RAND() LIMIT num_spaces;
    ...
    spaces_loop: LOOP
        ...
        DECLARE security_groups_cursor CURSOR FOR SELECT id FROM security_groups
            WHERE name LIKE '{{.Prefix}}-security-group-%' ORDER BY RAND() LIMIT num_security_groups_per_space;
...
BEGIN
    ...
    DECLARE org_id INT;
    DECLARE finished BOOLEAN DEFAULT FALSE;
    DECLARE orgs_cursor CURSOR FOR SELECT id FROM organizations WHERE name LIKE '{{.Prefix}}-org-%' ORDER BY RAND() LIMIT num_orgs;
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET finished = TRUE;
...
BEGIN
    ...
    DECLARE finished BOOLEAN DEFAULT FALSE;
    DECLARE orgs_cursor CURSOR FOR SELECT guid FROM organizations WHERE name LIKE '{{.Prefix}}-org-%' ORDER BY RAND() LIMIT num_orgs;
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET finished = TRUE;
    ...
    org_loop: LOOP
        ...
        IF finished THEN
            LEAVE org_loop;
        END IF;
        SELECT guid FROM isolation_segments WHERE name LIKE '{{.Prefix}}-isolation-segment-%' ORDER BY RAND() LIMIT 1 INTO v_isolation_segment_guid;
